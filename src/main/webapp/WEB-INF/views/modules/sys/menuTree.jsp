		...
		<c:forEach items="${menuList}" var="menu" varStatus="idxStatus">
			<c:if test="${menu.parent.id eq (not empty param.parentId ? param.parentId:1)&&menu.isShow eq '1'}">
				<c:if test="${not empty menu.href}">
					<div class="accordion-heading accordion-yyl">
						<a class="accordion-toggle" data-parent="#menu-${param.parentId}" target="${not empty menu.target ? menu.target : 'mainFrame'}" href="${fn:indexOf(menu.href, '://') eq -1 ? ctx : ''}${not empty menu.href ? menu.href : '/404'}" title="${menu.remarks}">
				...
				<c:if test="${empty menu.href}">
					<div class="accordion-group">
						<div class="accordion-heading">
							<%--<a data-href=".menu3-${menu2.id}" href="${fn:indexOf(menu2.href, '://') eq -1 ? ctx : ''}${not empty menu2.href ? menu2.href : '/404'}" target="${not empty menu2.target ? menu2.target : 'mainFrame'}" ><i class="icon-${not empty menu2.icon ? menu2.icon : 'circle-arrow-right'}"></i>&nbsp;${menu.name}</a>--%>
									...
									<li>
										...
										<ul class="nav nav-list hide" style="margin:0;padding-right:0;">
											...
											<li class="menu3-${menu2.id} hide">
												...
												<%--<ul class="nav nav-list hide" style="margin:0;padding-right:0;">--%>
													<%--<c:forEach items="${menuList}" var="menu4"><c:if test="${menu4.parent.id eq menu3.id&&menu4.isShow eq '1'}">--%>
														<%--<li class="menu4-${menu3.id} hide">--%>
															<%--<a href="${fn:indexOf(menu4.href, '://') eq -1 ? ctx : ''}${not empty menu4.href ? menu4.href : '/404'}" target="${not empty menu4.target ? menu4.target : 'mainFrame'}" ><i class="icon-${not empty menu4.icon ? menu4.icon : 'circle-arrow-right'}"></i>&nbsp;${menu4.name}</a>--%>
														<%--</li>--%>
													<%--</c:if></c:forEach>--%>
												<%--</ul>--%>
											</li>
