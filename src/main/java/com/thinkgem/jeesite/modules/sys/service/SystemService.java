...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void updatePsw(User user) {
		User dUser = userDao.get(user.getId());
		dUser.setPassword(user.getNewPassword());
		userDao.updatePasswordById(dUser);
		//清除缓存
		UserUtils.clearCache(user);
	}

	@Transactional(readOnly = false)
	public void updateLoginFlg(User user) {
		user.setLoginFlag(user.getLoginFlag());
		userDao.updateUserLoginFlg(user);
		//清除缓存
		UserUtils.clearCache(user);
	}

	public List<Role> findPage(Role role) {
		List<Role> allList = roleDao.findList(role);
